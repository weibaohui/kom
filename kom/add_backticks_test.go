...
func TestAddBackticks(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Nested fields",
			input:    "SELECT metadata.namespace FROM pod WHERE metadata.creationTimestamp>'2024-01-01'",
			expected: "SELECT metadata.namespace FROM pod WHERE `metadata.creationTimestamp` > '2024-01-01'",
		},
		// 运算符测试
		{
			name:     "Operators",
