...
3. MCP支持：支持多集群的MCP管理，内置45种工具，支持SSE模式。可通过大模型实现对任意资源的增删查描述操作。支持超过百种组合操作。
...
| **动态资源管理**    | `get_k8s_resource`           | 获取动态资源                                  |
|               | `describe_k8s_resource`      | 描述动态资源                                  |
|               | `delete_k8s_resource`        | 删除动态资源                                  |
|               | `list_k8s_resource`          | 列出动态资源                                  |
|               | `list_k8s_event`             | 列出动态资源事件                                |
